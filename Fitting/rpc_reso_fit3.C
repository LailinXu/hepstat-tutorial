...
void rpc_reso_fit3() {
  ...
  ROOT::Fit::BinData data(nbinsx*nbinsy*nbinsz, nd);
  double xx[3];
  for(int ix=1; ix < nbinsx+1; ix++) {
    float vx = h_s->GetXaxis()->GetBinCenter(ix);
    xx[0] = vx;
    for(int iy=1; iy < nbinsy+1; iy++) {
      float vy = h_s->GetYaxis()->GetBinCenter(iy);
      xx[1] = vy;
      for(int iz=1; iz < nbinsz+1; iz++) {
        float vz = h_s->GetZaxis()->GetBinCenter(iz);
        float v = h_s->GetBinContent(ix, iy, iz);
        float ev = h_s->GetBinError(ix, iy, iz);
        xx[2] = vz;
  ...
  ROOT::Math::WrappedMultiTF1 wf(*g_s,3);
  ...
  for(int ip=0; ip < parSettings.size(); ip++) {
    std::cout << "Parameter: " << parSettings[ip].Name() << " Value: " << parSettings[ip].Value() << " Lower: " << parSettings[ip].LowerLimit() << " Upper: " << parSettings[ip].UpperLimit() << " StepSize: " << parSettings[ip].StepSize() << std::endl;
  }

  // ret = fitter.Fit(data)
  bool ret = fitter.LikelihoodFit(data);
  ...
  g_s->SetFitResult(res);
  // test fit p-value (chi2 probability)
  double prob = res.Prob();
  std::cout <<" Chi2 p-value: " << prob << std::endl;
