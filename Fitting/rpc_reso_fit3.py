...
nbinsx, xmin, xmax = 6, -3, 3
nbinsy, ymin, ymax = 6, -3, 3
nbinsz, zmin, zmax = 6, -3, 3
...
np = nbinsx*nbinsy*nbinsz
data = R.Fit.BinData(np, 3)
xx = array.array("d", [0]*3)
for ix in range(1, nbinsx+1):
  for iy in range(1, nbinsy+1):
    for iz in range(1, nbinsz+1):
      xx[0] = h_s.GetXaxis().GetBinCenter(ix)
      xx[1] = h_s.GetYaxis().GetBinCenter(iy)
      xx[2] = h_s.GetZaxis().GetBinCenter(iz)
      v = h_s.GetBinContent(ix, iy, iz)
      ev = h_s.GetBinError(ix, iy, iz)
      data.Add(xx, v, ev)

fitter = R.Fit.Fitter()
wf = R.Math.WrappedMultiTF1(g_s, 3)
fitter.SetFunction(wf)
#ret = fitter.Fit(data)
ret = fitter.LikelihoodFit(data)
res = fitter.Result()
res.Print(R.cout)
